...
物理化学部コンピュータ班で開発している、ニコニコ動画風の流れるコメント×Twitter風投稿を組み合わせたWebアプリです。この資料は次の代に引き継ぐために書いています。分からないことがあれば、まずこのファイルを読んでから触ってください。前の代から直接教わっていなくても、このREADMEだけで管理・変更ができるように書いてあります。管理したい時・変更を加えたい時は、まず[アカウント一覧](#アカウント一覧)を見て、共用アカウントで自分自身にログインして作業してください（パスワードは部室の物化部ノートか「物化部現役」LINEグループのノートにあります）。初めて触る時は、[代替わりチェックリスト](#代替わりチェックリスト自分で管理を始める時にやること)を上から順番に進めれば準備が整います。
...
- **作り方**: 全てClaude（AI）とのバイブコーディングで作っています。コードの意味が分からない部分があれば、Claudeにこのリポジトリを読み込ませて聞くのが一番早いです。
...
- [ ] 部室の物化部ノート、または「物化部現役」グループLINEのノートから、このアカウントのパスワードを自分で確認する
- [ ] 実際にこのアカウントで、GitHub・Firebase両方にログインできることを確認する
- [ ] [データのバックアップと復旧方法](#データのバックアップと復旧方法)の「方法2」で、最新のJSONバックアップを一度取っておく
- [ ] （推奨）[権限の引き継ぎ方法](#権限の引き継ぎ方法詳細)に沿って、自分個人のアカウントもGitHub・Firebase両方にCollaborator/編集者として追加しておく（共用アカウントでログインしてさえいれば、これも自分だけでできます）
- [ ] 公開ページ（`https://omochinomocchili.github.io/nicotwi-bukka/`）から実際に投稿し、画面に反映されることを確認する
- [ ] [GitHubでの更新方法](#githubでの更新方法)を実際に1回試す（コメントを1行足すだけでも良い）
- [ ] [復旧方法](#データのバックアップと復旧方法)に一通り目を通しておく

これで、いつでも自分の手で管理・変更ができる状態になります。以降の章は、実際に作業する時に読む詳しい参考資料です。

## アカウント一覧

このアプリの運用に使うのは、部活全体で共用している次の1アカウントだけです。GitHub・Firebaseとも、このアカウントでログインします。**管理したい時・変更を加えたい時は、自分でパスワードを確認してこのアカウントにログインしてください。**
...
共用アカウントのパスワードだけに頼ると、ノートが見つからない・共用アカウントに何かあった、という時にログインできなくなります。自分個人のアカウントもCollaborator・編集者として追加しておけば、そちらからでもアクセスできるので、管理を始めたら一緒にやっておくことを推奨します。
